...
async def get_feeds(comic_list: list[Comic], hash_seed : int, **kwargs) -> list[FeedParserDict]:
...
def main(comics: Collection, hash_seed: int, webhook_url: str):
    ...
    feeds = asyncio.get_event_loop().run_until_complete(get_feeds(comic_list, hash_seed, timeout=timeout))
    print("done")
